...
```{r , eval=FALSE, comment='#'}
d <- left_join(persons, households) %>%
  mutate( IllegalVenez = ifelse( (P1894 == "5" &
                   P756S3 == "3"),
                  paste("Venz_SinDocumento"), 
                  paste("other")) )

# table(d$IllegalVenez, useNA="ifany")
```
]
...
```{r , eval=FALSE, comment='#'}
sjPlot::plot_xtab(x= d$P9050,
                  grp = d$IllegalVenez,
                  type = "bar" ,
                  weight.by = as.numeric(d$FEX_C),
                  # sort.frq =  "asc",
                  # bar.pos = "stack",
                  wrap.labels = 40,
                  show.n = FALSE,
                 ...
                 # summary.pos = "r",
                  coord.flip = TRUE) +
        labs(title = paste0(sjlabelled::get_label(households$P9050)),
             subtitle = "Crossed with Documentation Status",
             x = "",
             y = "",
