...
knitr::kable(Origin %>%  
  arrange(desc(DisplacedAcrossBorders)) %>%
    select(CountryOriginName,DisplacedAcrossBordersRound ) %>%
  head(10), format = 'html')
```
]
...
plot <- ggplot(data = Origin, 
  ...
  geom_bar(
           ...
           # default behavior is to count the rows for each x value (i.e. stat = "count")
           # stat = "identity" in geom_bar() tells ggplot2 to skip the aggregation
           # aka you provided the y values - an alternative is to use geom_col
           stat = "identity", 
           ## indicate the color to use to fill the bar - unhcr blue obviously!
           fill = "#0072bc")  
...
### Create Indicators - Proportion of PoC living below the national poverty line
